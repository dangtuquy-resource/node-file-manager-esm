...
router.put("/api/options", function*() {
    ...
    if (!type) {
        ...
        this.body = yield FileManager.toggleShowAllFiles();
    } else if (type === "GET_SHOW_ALL_FILES") {
        this.body = yield FileManager.showAllFiles();
...
router.post(
    ...
    function*() {
        ...
        if (!type) {
            ...
            var archive = p;
            yield* FileManager.archive(
                src,
                archive,
                C.data.root,
                !!this.request.body.embedDirs
            );
